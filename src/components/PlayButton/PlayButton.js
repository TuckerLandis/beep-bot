...
function PlayButton(props) { //// <--------<------<-----<------The play Button -------------------------- ////> nned to abstract, pass in steps?
    ...
    function startStop() {
        ...
        // if i can insert an incrementing value to the varname 'seq', i could only start a specific sequence, like props.beep_id
        ...
        if (!isPlaying) {  // starts Tone if stopped
            ...
            // Tone.Context.stop() // attempt to stop audio context for play button instance doesn't fix double play bug
        }
